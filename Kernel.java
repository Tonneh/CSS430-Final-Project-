...
private
  ...
  // Interrupt requests
public
  final static int INTERRUPT_SOFTWARE = 1; // System calls
public
  final static int INTERRUPT_DISK = 2; // Disk interrupts
public
  final static int INTERRUPT_IO = 3; // Other I/O interrupts
  // System calls
public
  final static int BOOT = 0; // SysLib.boot( )
public
  final static int EXEC = 1; // SysLib.exec(String args[])
public
  final static int WAIT = 2; // SysLib.join( )
public
  final static int EXIT = 3; // SysLib.exit( )
public
  final static int SLEEP = 4; // SysLib.sleep(int milliseconds)
public
  final static int RAWREAD = 5; // SysLib.rawread(int blk, byte b[])
public
  final static int RAWWRITE = 6; // SysLib.rawwrite(int blk, byte b[])
public
  final static int SYNC = 7; // SysLib.sync( )
public
  final static int READ = 8; // SysLib.cin( )
public
  final static int WRITE = 9; // SysLib.cout( ) and SysLib.cerr( )
  // System calls to be added in Assignment 4
public
  final static int CREAD = 10; // SysLib.cread(int blk, byte b[])
public
  final static int CWRITE = 11; // SysLib.cwrite(int blk, byte b[])
public
  final static int CSYNC = 12; // SysLib.csync( )
public
  final static int CFLUSH = 13; // SysLib.cflush( )
  // System calls to be added in Project
public
  final static int OPEN = 14; // SysLib.open( String fileName )
public
  final static int CLOSE = 15; // SysLib.close( int fd )
public
  final static int SIZE = 16; // SysLib.size( int fd )
public
  final static int SEEK = 17; // SysLib.seek( int fd, int offest,
                              //              int whence )
public
  final static int FORMAT = 18; // SysLib.format( int files )
public
  final static int DELETE = 19; // SysLib.delete( String fileName )
...
private
  ...
  // Synchronized Queues
private
  static SyncQueue waitQueue; // for threads to wait for their child
private
  static SyncQueue ioQueue; // I/O queue
private
  final static int COND_DISK_REQ = 1; // wait condition
private
  final static int COND_DISK_FIN = 2; // wait condition
  // Standard input
private
  ...
  // The heart of Kernel
public
  static int interrupt(int irq, int cmd, int param, Object args) {
    TCB myTcb;
    switch (irq) {
    case INTERRUPT_SOFTWARE: // System calls
      ...
      case READ:
        ...
        if ((myTcb = scheduler.getMyTcb()) != null) {       // check if TCB is null 
            ...
            if (ftEnt != null) {                            // if entry isnt null
                return fs.read(ftEnt, (byte[])args);         // return what read returned (amount read or -1 for error)
            }
        }
      ...
      case WRITE:
        ...
        if ((myTcb = scheduler.getMyTcb()) != null) {       // check if TCB is null 
            FileTableEntry ftEnt = myTcb.getFtEnt(param);   // create an entry 
            if (ftEnt != null) {                            // if entry isnt null
                return fs.write(ftEnt, (byte[])args);       // return what write returned (amount written or -1 for error)
            }
        }
        return ERROR; 
      ...
      case OPEN: // to be implemented in project
        if ((myTcb = scheduler.getMyTcb()) != null) { // check if tcb is null
          String[] s = (String[])args;                // get arguments as string 
          FileTableEntry ent = fs.open(s[0], s[1]);   // open an entry
          int fd = myTcb.getFd(ent);                  // get fd 
          return fd;                                  // return 
        } else {
          return ERROR;
        }
      case CLOSE: // to be implemented in project
        if ((myTcb = scheduler.getMyTcb()) != null) {   // check if tcb is null
          ...
          if (ent == null || fs.close(ent) == false) {  // if entry is null or fs.close returns false 
            return ERROR;                               // return error
          }
          if (myTcb.returnFd(param) != ent) {
            return ERROR;                               // if the ent returned isn't same then return error
          }
          return OK;
        }
        return ERROR;
      case SIZE: // to be implemented in project
        if ((myTcb = scheduler.getMyTcb()) != null) {     // check if tcb is null
          ...
          if (ftEnt != null) {                            // check if entry is null
            return fs.fsize(ftEnt);                       // return size 
          }
        }
        return ERROR;
      case SEEK: // to be implemented in project
        if ((myTcb = scheduler.getMyTcb()) != null) {         // check if tcb is null
          ...
          FileTableEntry ftEnt = myTcb.getFtEnt(param);
          if (ftEnt != null) {                                // check if entry is null
            return fs.seek(ftEnt, seekArgs[0], seekArgs[1]);  // return the int returned from seek
          }
        }
        return ERROR;
      case FORMAT: // to be implemented in project
        return (fs.format(param) == true) ? OK : ERROR;         // if tree return ok else return error
      case DELETE: // to be implemented in project
        return (fs.delete((String)args) == true) ? OK : ERROR;  // if tree return ok else return error
      }
      return ERROR;
...
private
  static int sysExec(String args[]) {
